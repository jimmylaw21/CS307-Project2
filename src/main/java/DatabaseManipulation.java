...
public class DatabaseManipulation implements DataManipulation {
    ...
    public String Query (String query){
        ...
        switch (strings[0]){
            case "enterprise":
                ...
                columnLabel = "name";
            case "model":
                sql = "select model from model where id = ? ;";
                columnLabel = "model";
            case "center":
                sql = "select name from center where id = ? ;";
                columnLabel = "name";
            case "staff":
                sql = "select name from model where number = ? ;";
                columnLabel = "name";
            default:
                sb = "invalid query";
        }
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            while(resultSet.next()){
                sb = (String.format(resultSet.getString(columnLabel)));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public int findStockInQuantity(String model, String center) {
        int num = 0;
        String sql = "select quantity from staff where model = ? and center = ?;";
        try {
            PreparedStatement preparedStatement = con.prepareStatement(sql);
    ...
    public void Insert(String table, String... values) {
        switch (table){
            case "center":
                String sql1 = "insert into center (id,name) " +
                "values (?,?)";
                try {
            ...
            case "enterprise":
                String sql2 = "insert into enterprise (id,name,country,city,supply_center,industry) " +
                        "values (?,?,?,?,?,?)";
                try {
            ...
            case "staff":
                String sql3 = "insert into staff (id,name,age,gender,number,supply_center,mobile_phone,type) " +
                        "values (?,?,?,?,?,?,?,?)";
                try {
            ...
            case "model":
                String sql4 = "insert into model (id,number,model,name,unit_price) " +
                        "values (?,?,?,?,?)";
                try {
    ...
    public int updateOrderContract(String Contract_number, int number, int num) {
        int resultSet = 0;
        String sql = "update orders set supply_center = ? where quantity = ?;";
        try {
            for (int i = 0; i < num; i++) {
                PreparedStatement preparedStatement = con.prepareStatement(sql);
                preparedStatement.setString(1,Contract_number);
                preparedStatement.setInt(2,number +i);
                resultSet = preparedStatement.executeUpdate();
                System.out.println(preparedStatement.toString());
            }
        } catch (SQLException e) {
    ...
    public String orderWithSalesman(int num1,int num2,int num3,int num4,int num5,int num6,int num7,int num8,int num9,int num10) {
        StringBuilder sb = new StringBuilder();
        String sql = "select orders_id from orders where Salesman_number in(?,?,?,?,?,?,?,?,?,?);";
        try {
            PreparedStatement preparedStatement = con.prepareStatement(sql);
            preparedStatement.setInt(1,num1);
            preparedStatement.setInt(2,num2);
            preparedStatement.setInt(3,num3);
            preparedStatement.setInt(4,num4);
            preparedStatement.setInt(5,num5);
            preparedStatement.setInt(6,num6);
            preparedStatement.setInt(7,num7);
            preparedStatement.setInt(8,num8);
            preparedStatement.setInt(9,num9);
            preparedStatement.setInt(10,num10);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                sb.append(resultSet.getString("orders_id"));
                sb.append(System.lineSeparator());
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static void updateModel(int id,String number, String model,String name, int unit_price){
        String sql = "update model set id = ?,number = ?,model = ?, name = ? , unit_price = ? " +
                "where id = ?";
        try {
            PreparedStatement preparedStatement = con.prepareStatement(sql);
            preparedStatement.setInt(1,id);
            ...
            preparedStatement.setInt(5,unit_price);
            preparedStatement.executeUpdate();
            con.commit();
        } catch (SQLException e) {
            e.printStackTrace();
